...
The model path reuses the established OpenRouter-compatible OpenAI client and uses the V7-specific `openai/gpt-5.1` model without coupling to Weekly Intelligence. The compact input is `{ contract, evidenceSnapshot }`; the contract lists required dimensions, state taxonomy, overall labels, and required synthesis/recommendation sections. Generation uses strict provider-side JSON Schema structured output derived from the same bounded V7 contract. The provider schema is only a generation constraint: the server parser remains authoritative before persistence. No unrelated internal intelligence or unnecessary PII is sent. Authoritative output failures are logged only with a bounded, server-controlled `validationReason`; raw output, parser messages, prompts, evidence, and response bodies are never diagnostic metadata.
