...
insert into public.validation_subjects(
  id,
  owner_id,
  creation_origin,
  label
...
insert into public.validation_hypotheses(
  id,
  owner_id,
  subject_id,
  status
...
insert into public.validation_hypothesis_versions(
  id,
  owner_id,
  subject_id,
  hypothesis_id,
...
insert into public.validation_experiment_versions(
  id,
  owner_id,
  subject_id,
  experiment_id,
...
insert into public.validation_interview_plan_versions(
  id,
  owner_id,
  subject_id,
  experiment_id,
...
-- ---------------------------------------------------------------------
--
-- The representative-state argument is captured immediately before the
-- promotion RPC. This models a fresh server preparation for ordinary
-- fixture promotions.
--
-- The dedicated stale-representative concurrency scenario in the Node
-- runner will intentionally capture this state earlier and reuse the
-- stale value after a competing candidate is committed.

create function public.b31_test_promote(
  i integer,
  pol text,
  fp text
)
...
begin
  if has_function_privilege(
    'anon',
    'public.validation_promote_customer_interview_evidence(uuid,uuid,uuid,uuid,text,text,text,jsonb,jsonb,text,text,text,text,text)',
    'execute'
  )
  ...
  then
    raise exception 'browser promotion grant';
  end if;
  ...
  then
    raise exception
      'service role missing promotion grant';
  end if;
  ...
  then
    raise exception
      'browser representative-state grant';
  end if;
  ...
  then
    raise exception
      'service role missing representative-state grant';
  end if;
end
$$;
...
select public.b31_test_promote(
  5,
  'supporting',
  repeat('5',64)
);
...
begin
  begin
    perform public.validation_promote_customer_interview_evidence(
      '10000000-0000-0000-0000-000000000001',
      'b0000000-0000-0000-0000-000000000005',
    ...
    raise exception
      'supersession duplicated source';
...
begin
  ...
  then
    raise exception
      'classification finality failed';
  end if;
end
$$;
...
-- ---------------------------------------------------------------------
-- Projection privacy, atomic rollback and exact retry
-- ---------------------------------------------------------------------
...
begin
  ...
  if exists(
    ...
    from public.problem_observations
    where
      source_evidence like '%PRIVATE RAW%'
      or source_author_id is not null
  ...
  begin
    perform public.b31_test_promote(
      4,
      'supporting',
      repeat('4',64)
    );
...
begin
  ...
  if
    (
      select count(*)
      from public.problem_evolution_snapshots
    ) <> 0
    or
    (
      select count(*)
