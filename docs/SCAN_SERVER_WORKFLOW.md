...
`SCAN_SERVER_WORKFLOW_ALLOWED_USER_IDS` is only a closed-rollout access gate. It does not grant quota, unlimited Scans, an internal-tester assignment, or administrator authority. Exceptional unlimited access is represented by an active, unexpired `application_user_access` row for the authenticated UUID with role `internal_tester` and `unlimited_scans = true`; the database acceptance function checks it independently. The service role is only the trusted technical executor, not an end-user entitlement.
