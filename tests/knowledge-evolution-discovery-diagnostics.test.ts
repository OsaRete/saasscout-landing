...
import { runKnowledgeEvolutionDiscoveryDiagnostics } from "../lib/knowledge/evolution/discovery-diagnostics.ts";
...
test("diagnostics disabled means caller can skip repository execution", () => {
  const previous = process.env.KNOWLEDGE_EVOLUTION_DIAGNOSTICS;
  delete process.env.KNOWLEDGE_EVOLUTION_DIAGNOSTICS;
  const { calls } = clientFor({ discovered_problems: [{ problem_title: "Skipped" }] });
...
test("diagnostics enabled assesses a discovered problem without changing workflow-shaped output", async () => {
  const logger = silenceConsole();
  try {
    const workflowOutput = { success: true, problems: [{ id: "p1" }] };
    const { client } = clientFor({
      discovered_problems: [{ problem_title: "Client reporting bottlenecks", created_at: "2026-06-01", pain_score: 8, source_evidence: "reported by agencies" }],
    });

    const result = await runKnowledgeEvolutionDiscoveryDiagnostics({
      client,
    ...
    assert.deepEqual(workflowOutput, { success: true, problems: [{ id: "p1" }] });
  } finally {
    logger.restore();
  }
});

test("repository failure is logged safely and does not throw", async () => {
  const logger = silenceConsole();
  try {
    const { client } = clientFor({
      ...
      discovered_problems: new Error("database offline"),
    });

    const result = await runKnowledgeEvolutionDiscoveryDiagnostics({
      client,
      problems: [{ problem_title: "Offline assessment" }],
    });
...
test("multiple discovered problems and partial source failures are reported", async () => {
  const logger = silenceConsole();
  try {
    const { client } = clientFor({
