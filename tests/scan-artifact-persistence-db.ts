...
test("read and corruption probes classify owner scope and stored-row inconsistencies", async () => { truncateLocalOnly(); const f = await fixture("000000000601"); json(rpcSql(f), "service_role"); assert.equal(countRows(` where owner_id=${q(scanArtifactOwnerB)}::uuid and artifact_id=${q(f.artifact.artifactId)}`), "0"); const valid = json(`select artifact_payload from public.scan_intelligence_artifacts where owner_id=${q(f.ownerId)}::uuid and artifact_id=${q(f.artifact.artifactId)};`); assert.equal(canonicalSerializeScanIntelligenceArtifact(valid.artifact_payload), canonicalSerializeScanIntelligenceArtifact(f.artifact)); const corruptions = [
  ...
  for (const [name, set] of corruptions) { psql("alter table public.scan_intelligence_artifacts disable trigger scan_artifacts_append_only;"); try { psql(`update public.scan_intelligence_artifacts set ${set};`); const row = json("select * from public.scan_intelligence_artifacts;"); assert.throws(() => validateStoredScanArtifactRow(row), (e) => e instanceof ScanArtifactPersistenceError && e.code === "scan_artifact_persistence_corrupt", name); } finally { psql("alter table public.scan_intelligence_artifacts enable trigger scan_artifacts_append_only; truncate table public.scan_intelligence_artifacts restart identity;"); json(rpcSql(f), "service_role"); } } });
...
test("real PostgreSQL concurrent equal and conflicting RPC calls serialize deterministically", async () => { truncateLocalOnly(); const equal = await fixture("000000000701"); const call = () => jsonAsync(rpcSql(equal), "service_role"); const equalResults = await settledJsonPair([call(), call()]); assert.deepEqual(equalResults.map(r => r.status).sort(), ["inserted", "replayed"]); assert.equal(countRows(), "1"); assert.equal(inspectStoredRow().artifact_hash, equal.artifact.integrity.artifactHash); truncateLocalOnly(); const a = await fixture("000000000702"); const b = await fixture("000000000703"); const conflictResults = await settledJsonPair([jsonAsync(rpcSql(a), "service_role"), jsonAsync(rpcSql({ ...b, ownerId: a.ownerId, idempotencyKey: a.idempotencyKey }), "service_role")]); assert.deepEqual(conflictResults.map(r => r.status).sort(), ["conflict", "inserted"]); assert.equal(countRows(), "1"); assert.equal(inspectStoredRow().artifact_id, a.artifact.artifactId); });


test("local inspection helpers never impersonate service_role", () => { const source = readFileSync(new URL(import.meta.url), "utf8"); assert.doesNotMatch(source, /psql\([^\n]*scan_intelligence_artifacts[^\n]*, "service_role"/); assert.doesNotMatch(source, /json\([^\n]*scan_intelligence_artifacts[^\n]*, "service_role"/); });
