...
function solution(id="pasted-evidence-001") { const e=claim(id); const cat=(category:string,suitability:number,suitabilityBand:string)=>({ category, suitability, suitabilityBand, rationale:inf, advantages:[e,inf], limitations:[e], prerequisites:[inf] }); return validateSolutionIntelligenceOutput({ version:"scan-solution-intelligence@1", problemFraming:e, evaluatedCategories:[cat("software_product",0.5,"possible"), cat("productized_service",0.7,"strong"), cat("validate_first",0.9,"best_fit")], recommendedCategory:"validate_first", secondaryCategory:"productized_service", recommendation:inf, existingSolutionAssessment:{ knownAlternatives:[{ nameOrCategory:"Observed Competitor", alternativeType:"direct_competitor", observedStrengths:[e], observedWeaknesses:[inf], evidenceRefs:[{ evidenceId:id }] }], evidenceCoverage:"limited", whatAppearsValidated:[e], whatAppearsPoorlySolved:[e], replacementRisk:inf }, innovationAssessment:{ innovationMode:"incremental_improvement", verifiedFoundation:[e], proposedDifferentiation:[inf], unverifiedAssumptions:[inf], feasibilityConstraints:[e], noveltyRisk:"moderate" }, validationReadiness:{ readiness:"solution_validation_ready", knownFacts:[e], criticalUnknowns:[inf], cheapestNextTest:"customer_interviews", testRationale:inf, successSignal:inf, failureSignal:inf }, keyAssumptions:[inf], risks:[e], nextValidationAction:inf }, { evidenceIds:[id] }); }
...
test("production-shaped populated claims parse, normalize, validate, and map to a deterministic persistence artifact", async () => { const a=await makeArtifact(); const b=await makeArtifact(); assert.deepEqual(a.artifact,b.artifact); assert.equal(a.artifact.version,"scan-intelligence-artifact@1"); assert.equal(a.artifact.artifactId,"scan-artifact-00000000-0000-4000-8000-000000000000"); assert.equal(a.artifact.execution.status,"completed"); validateScanIntelligenceArtifact(a.artifact); assert.equal(verifyScanIntelligenceArtifactIntegrity(a.artifact), true); assert.equal(a.artifact.solutionIntelligence.evaluatedCategories[0].advantages.length,2); assert.equal(a.artifact.solutionIntelligence.innovationAssessment.verifiedFoundation.length,1); assert.equal(Object.isFrozen(a.artifact.solutionIntelligence.validationReadiness.knownFacts), true); assert.equal(canonicalSerializeScanIntelligenceArtifact(a.artifact), canonicalSerializeScanIntelligenceArtifact(b.artifact)); assert.equal(a.artifact.integrity.artifactHash,b.artifact.integrity.artifactHash); });
