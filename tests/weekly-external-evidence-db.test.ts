...
test("W-B migration preserves server-only access and permits one observation per source per run", { skip: !databaseUrl }, () => {
  ...
  execFileSync("psql", [databaseUrl!, "-v", "ON_ERROR_STOP=1", "-c", "begin;", "-f", "supabase/migrations/20260628000000_create_historical_application_schema.sql", "-f", "supabase/migrations/20260816000000_weekly_external_evidence_contract.sql", "-f", "supabase/migrations/20260821000000_weekly_beta_stabilization.sql", "-c", "select evidence_id, freshness_class, origin_class from public.weekly_sources limit 0;", "-c", "select execution_mode, external_provider_state, external_sources_persisted, source_degraded from public.weekly_intelligence_runs limit 0;", "-c", "rollback;"], { stdio: "pipe" });
