...
export function adaptPersistedPromotionRows(rows: PersistedPromotionRows): PreparedObservation[] {
  ...
  return rows.observations.map((row) => {
    ...
    return { input, identity: { provenanceCanonicalProblemId: text(context.canonical_problem_id) ?? text(context.canonicalProblemId), subjectLabel: text(subject.label), hypothesisProblemClaim: text(hypothesis.problem_claim) }, specificity: specificity(content, input.modality), statementKind: content.statement_kind === "direct_quote" ? "direct_quote" : content.statement_kind === "summary" ? "summary" : null, contentLength: contentText.length, observedAt: text(row.observed_at) ?? "" };
  });
}
