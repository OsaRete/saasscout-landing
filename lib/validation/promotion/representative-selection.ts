...
export function selectValidationRepresentatives(candidates: RepresentativeCandidate[]): RepresentativeSelection[] {
  ...
  return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([groupKey, members]) => { const sorted = [...members].sort(compare); return { groupKey, representativeObservationId: sorted[0].observationId, memberObservationIds: members.map((item) => item.observationId).sort() }; });
}
