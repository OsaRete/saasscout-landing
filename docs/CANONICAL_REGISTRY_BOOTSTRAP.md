...
## V8-B0.2.1 — safe apply failure diagnostics

The first controlled production apply failed atomically with the formerly generic `canonical_bootstrap_transaction_failed`. Post-failure verification found zero canonical problems, aliases, activation-ledger rows, and canonicalized observations, so no partial activation or manual cleanup remained. Static migration-chain review proved the failure: the RPC used `jsonb_array_elements(... )` followed by `value::text::uuid`; PostgreSQL's JSONB-to-text conversion preserves the JSON string quotes, so the first observation membership UUID cast fails before mutation. Migration `20260912010000_canonical_bootstrap_apply_diagnostics.sql` replaces those conversions with `jsonb_array_elements_text(... )` and direct text-to-UUID casts.

The forward migration also retains the single transaction, advisory transaction lock, exact ownership verification, fixed `SECURITY DEFINER` search path, and service-role-only execute grant while mapping unexpected failures to bounded stages: canonical insert, ledger insert, alias insert, observation link, post-link verification, or internal integrity. The CLI accepts only the explicit safe error allowlist from a PostgREST `message`; malformed, unknown, or arbitrary remote bodies collapse to `canonical_bootstrap_transaction_failed` and are never printed.

The effective schema permits the supplied canonical columns and `active` status, `title` aliases, the current ledger payload, and server-owned `canonical_problem_id`/`updated_at` updates; no migration adds an immutability trigger to `problem_observations`, changes these identity/status constraints, or rejects service-role writes. Text fields impose no candidate-length format constraint, the `bootstrap:<ruleVersion>:<candidateId>` key is valid and uniquely indexed, and the global unique `problem_aliases(normalized_alias)` index is a valid inference target for `ON CONFLICT (normalized_alias)`. The ledger's omission of `cause_consequence_identity_audit_rule_version` is incomplete provenance but not a first-activation correctness or idempotency defect because that rule remains committed inside the candidate snapshot hash and overall reviewed plan hash; additive ledger provenance can be considered separately.

There are no B0.1, B0.1.1, B0.1.2, B0.1.3, eligibility, or candidate-snapshot changes. The reviewed plan remains five candidates (`cb1_1e47593fd365ef352633eab6`, `cb1_2e74cc331b42c9b041a22d40`, `cb1_3f4b214a7fbbf2f43e201eaa`, `cb1_62263ef1cb13f093024925cf`, and `cb1_e12233fe75f9f0099d11a3d9`) with hash `2b389d4e0d7c3854e3dd3eb4f96c7ebfb5687b43b531fa30f96eaad4964c7392`; `cb1_9a2d319d26e902747fbcbf7a` remains blocked. No production dry-run, apply, or write was performed for B0.2.1. The new migration requires controlled review before any retry, and the operator must perform and inspect a fresh dry-run even if it reproduces the same hash.
