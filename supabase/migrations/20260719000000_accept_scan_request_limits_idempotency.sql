...
BEGIN
  IF auth.uid() IS NULL OR auth.uid() <> p_user_id THEN
    RETURN QUERY SELECT NULL::uuid, NULL::text, false, 'unauthorized'::text;
    RETURN;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.accept_scan_request(uuid, text, text, text, text) TO authenticated;
