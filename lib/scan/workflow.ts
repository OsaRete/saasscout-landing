...
import { validateScanTrustedIntent } from "./trusted-intent.ts";
...
export type ScanWorkflowEvidenceArtifactReference = Readonly<{ evidenceId: string; sourceKind: import("./evidence-ingestion.ts").ScanEvidenceSourceKind; trustClass: import("./evidence-ingestion.ts").ScanEvidenceTrustClass; privacyClass: import("./evidence-ingestion.ts").ScanEvidencePrivacyClass; characterCount: number; truncated: boolean; extractionStatus: import("./evidence-ingestion.ts").ScanEvidenceExtractionStatus; contentHash: string }>;
export type ScanWorkflowArtifactMappingContext = Readonly<{ evidenceManifest: readonly ScanWorkflowEvidenceArtifactReference[] }>;
...
export function validateScanWorkflowIntent(input: unknown): ScanWorkflowSafeIntent { try { return validateScanTrustedIntent(input) as ScanWorkflowSafeIntent; } catch { throw new ScanWorkflowError("scan_workflow_request_invalid", "input_validated", "4xx"); } }
export class ScanWorkflowError extends Error { readonly code: ScanWorkflowErrorCode; readonly stage: ScanWorkflowStage; readonly statusClass: string; readonly causeCategory?: string; readonly diagnostics?: import("./solution-intelligence-service.ts").SolutionGroundingFailureDiagnostics; constructor(code: ScanWorkflowErrorCode, stage: ScanWorkflowStage, statusClass: string, causeCategory?: string, diagnostics?: import("./solution-intelligence-service.ts").SolutionGroundingFailureDiagnostics) { super("The Scan workflow could not safely complete."); this.name="ScanWorkflowError"; this.code=code; this.stage=stage; this.statusClass=statusClass; this.causeCategory=causeCategory; this.diagnostics=diagnostics; } }
...
export async function executeScanWorkflow(input: ScanWorkflowInput, authorization: ScanWorkflowAuthorizationContext, deps: ScanWorkflowDependencies = defaultScanWorkflowDependencies): Promise<ScanWorkflowResult> {
  ...
  try {
    ...
    return Object.freeze({ version: SCAN_WORKFLOW_VERSION, executionId, status: "completed", startedAt: startedAt.toISOString(), completedAt: completedAt.toISOString(), intent, evidence: evidenceSummary(ing), problemIntelligence: problemOutput, problemDiagnostics, problemCalibration, solutionIntelligence: solutionOutput, solutionDiagnostics, processingHistory: rec.history(), technicalContext, artifactMappingContext: Object.freeze({ evidenceManifest: evidenceManifest(ing) }) });
  } catch (e) {
    const err = mapError(e, rec.activeStage() ?? "failed");
    if (rec.activeStage()) rec.fail(rec.activeStage() as ScanWorkflowStage, err.code);
    throw Object.freeze({ version: SCAN_WORKFLOW_VERSION, executionId, status: "failed", startedAt: startedAt.toISOString(), completedAt: deps.now().toISOString(), error: Object.freeze({ code: err.code, stage: err.stage, message: "The Scan workflow could not safely complete." as const, statusClass: err.statusClass, ...(err.causeCategory ? { causeCategory: err.causeCategory } : {}) }), ...(err.diagnostics ? { diagnostics:err.diagnostics } : {}), processingHistory: rec.history() }) satisfies ScanWorkflowFailureResult;
  }
}
...
export function scanWorkflowHttpStatusForFailure(failure: ScanWorkflowFailureResult) { if (failure.error.statusClass === "4xx") return 400; if (failure.error.statusClass === "422") return 422; if (failure.error.statusClass === "502") return 502; if (failure.error.code === "scan_workflow_configuration_failed") return 500; return 500; }
export function buildSafeScanWorkflowLog(input: { event:string; result?: ScanWorkflowResult; failure?: ScanWorkflowFailureResult }) { const h=input.result?.processingHistory ?? input.failure?.processingHistory ?? []; const tc=input.result?.technicalContext; return Object.freeze({ event:input.failure?.error.code === "scan_workflow_solution_grounding_failed" ? "scan_solution_grounding_validation_failed" : input.event, workflowVersion:SCAN_WORKFLOW_VERSION, executionId:input.result?.executionId ?? input.failure?.executionId, status:input.result?.status ?? input.failure?.status, failedStage:input.failure?.error.stage, errorCode:input.failure?.error.code, totalDurationMs:tc?.totalDurationMs ?? h.reduce((sum,r)=>sum+(r.durationMs ?? 0),0), completedStageCount:h.filter(r=>r.status==="completed").length, evidenceSourceCount:input.result?.evidence.sourceCount, evidenceSourceKindCounts:input.result?.evidence.sourceKindCounts, independentEvidenceCount:input.result?.evidence.independentSourceCount, problemGroundingCoverage:input.result?.problemDiagnostics.qualitySummary.groundingCoverage, problemReliabilityClassification:input.result?.problemCalibration.reliabilityClassification, solutionCategoryCount:input.result?.solutionDiagnostics.categoryCount, validationReadiness:input.result?.solutionDiagnostics.validationReadiness, stageDurations:Object.freeze(Object.fromEntries(h.map(r=>[r.stage,r.durationMs ?? 0]))), problemPromptVersion:tc?.problemPromptVersion, solutionPromptVersion:tc?.solutionPromptVersion, problemModel:tc?.problemModel, solutionModel:tc?.solutionModel, ...(input.failure?.diagnostics ?? {}) }); }
